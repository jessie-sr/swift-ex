...
void ExecutionService<T>::AddExecutionOrder(const AlgoExecution<T> &algoExecution)
{
  ...
  if (executionOrderMap.find(orderId) != executionOrderMap.end())
  {
    executionOrderMap.erase(orderId);
  }
...
void ExecutionOutputConnector<T>::Publish(const ExecutionOrder<T> &order, Market &market)
{
  ...
  std::string dataLine = string("ExecutionOrder: \n") + "\tProduct: " + product.GetProductId() + "\tOrderId: " + order.GetOrderId() + "\tTrade Market: " + tradeMarket + "\n" + "\tPricingSide: " + (order.GetSide() == BID ? "Bid" : "Offer") + "\tOrderType: " + order_type + "\t\tIsChildOrder: " + (order.IsChildOrder() ? "True" : "False") + "\n" + "\tPrice: " + std::to_string(order.GetPrice()) + "\tVisibleQuantity: " + std::to_string(order.GetVisibleQuantity()) + "\tHiddenQuantity: " + std::to_string(order.GetHiddenQuantity()) + "\n";
