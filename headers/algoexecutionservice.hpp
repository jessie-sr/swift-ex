...
template <typename T>
ExecutionOrder<T>::ExecutionOrder(const T &_product, PricingSide _side, string _orderId, OrderType _orderType, double _price, double _visibleQuantity, double _hiddenQuantity, string _parentOrderId, bool _isChildOrder) : product(_product)
{
  side = _side;
  orderId = _orderId;
...
template <typename T>
AlgoExecution<T>::AlgoExecution(const ExecutionOrder<T> &_executionOrder, Market _market) : executionOrder(_executionOrder), market(_market)
{
}
...
void AlgoExecutionService<T>::AlgoExecuteOrder(OrderBook<T> &_orderBook)
{
  ...
  if (algoExecutionMap.find(key) != algoExecutionMap.end())
  {
    algoExecutionMap.erase(key);
  }
